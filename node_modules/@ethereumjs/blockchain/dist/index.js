...
class Blockchain {
    ...
    async _init(genesisBlock) {
        ...
        if (!dbGenesisBlock) {
            ...
            const dbOps = [];
            dbOps.push((0, helpers_1.DBSetTD)(genesisBlock.header.difficulty.clone(), new ethereumjs_util_1.BN(0), genesisHash));
            (0, helpers_1.DBSetBlockOrHeader)(genesisBlock).map((op) => dbOps.push(op));
            (0, helpers_1.DBSaveLookups)(genesisHash, new ethereumjs_util_1.BN(0)).map((op) => dbOps.push(op));
        ...
        if (header && !header.coinbase.isZero()) {
            ...
            for (let round = 1; round <= 2; round++) {
                ...
                if (round === 1) {
                    ...
                    debug(`[Block ${header.number}] New clique vote: ${signer} -> ${beneficiary} ${nonce.equals(clique_1.CLIQUE_NONCE_AUTH) ? 'AUTH' : 'DROP'}`);
                }
                if (consensus) {
                    if (round === 1) {
    ...
    async _putBlockOrHeader(item) {
        await this.runWithLock(async () => {
            ...
            const isHeader = item instanceof block_1.BlockHeader;
            ...
            if (this._common.consensusAlgorithm() === common_1.ConsensusAlgorithm.Clique && ancientHeaderNumber) {
    ...
    async cliqueSignerInTurn(signer) {
        ...
        return number.addn(1).mod(new ethereumjs_util_1.BN(signers.length)).eqn(signerIndex);
    }
}
